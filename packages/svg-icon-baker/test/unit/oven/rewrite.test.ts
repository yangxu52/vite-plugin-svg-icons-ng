...
import type { BakeIssue } from '../../../src/types'
import type { RewriteOptions } from '../../../src/oven/types'
import { buildSvg, parseSvg } from '../../../src/oven/xml'
import { rewriteIds } from '../../../src/oven/rewrite'
...
describe('rewriteIds', () => {
  ...
  test('reports parse style failure and preserves original style content', async () => {
    ...
    try {
      const { rewriteIds: rewriteIdsWithMock } = await import('../../../src/oven/rewrite')
      const code = `<svg viewBox="0 0 10 10"><style>#a{fill:url(#g)}</style><defs><linearGradient id="g"><stop offset="0"/></linearGradient></defs><rect id="a" width="10" height="10"/></svg>`
