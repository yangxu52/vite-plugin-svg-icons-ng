...
import { VIRTUAL_IDS, VIRTUAL_NAMES_DEPRECATED, VIRTUAL_REGISTER, VIRTUAL_REGISTER_DEPRECATED, VIRTUAL_SPRITE } from '../constants'
...
function applyIconChange(ctx: PluginContext, server: ViteDevServer, file: string): boolean {
...
export function pluginConfigureServer(ctx: PluginContext, server: ViteDevServer): void {
  ...
  const onIconFileAddedOrRemoved = (file: string) => {
    applyIconChange(ctx, server, file)
  }
...
export function pluginHandleHotUpdate(ctx: PluginContext, hotUpdateCtx: HmrContext): [] | void {
  if (!applyIconChange(ctx, hotUpdateCtx.server, hotUpdateCtx.file)) {
    return
  }
  return []
}
